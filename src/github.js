...
class Github {
  ...
  async createBumpLibPullRequest({
    ...
    const cwd = './';
      const tree = await this.getSubPackageTree();
      console.log(tree);
      function recurseVias(lib, level=0) {
        console.log("  ".repeat(level) + lib);
        tree[lib].vias.forEach(child => recurseVias(child, level+1));
      }
      const depTree = JSON.stringify(recurseVias(pythonPackage), null, 2);
      console.log(depTree);
      process.exit(0);
